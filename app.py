...
# تعريف تعليمات النظام كمتغير عالمي
...
# كلاس لتخزين بيانات المريض باستخدام session_state
...
# تكوين نموذج Gemini AI بدون معلمة system_instruction
...
# تحليل البيانات الطبية باستخدام AI مع تمرير تعليمات النظام عند بدء جلسة الدردشة
...
6. ذكر الأمراض المحتملة إذا استمرت الحالة كما هي الآن أو إذا كانت الحالة مستقرة فلن تحدث أي مشاكل، مع ذكر الأسباب العلمية لهذه الأمراض بناءً على البيانات المقدمة."""
    try:
        # بدء جلسة الدردشة مع تمرير رسالة النظام كأول رسالة
        chat_session = model.start_chat(history=[{"role": "system", "content": SYSTEM_PROMPT}])
...
# استدعاء نموذج التنبؤ بأمراض القلب باستخدام gradio_client
...
# نموذج احتياطي محلي للتنبؤ بأمراض القلب
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio(
            ...
            index=["بيانات المريض", "مراقبة المؤشرات الحيوية", "تحليل مخاطر القلب"].index(st.session_state.page)
        )
    ...
    if page == "بيانات المريض":
        ...
        with st.form("patient_info_form"):
            ...
            with col2:
                patient_data["sex"] = st.radio("الجنس", options=["1", "0"],
                                                format_func=lambda x: "ذكر" if x == "1" else "أنثى",
                                                index=0 if patient_data.get("sex", "1") == "1" else 1)
    ...
    # صفحة تحليل مخاطر القلب
        ...
        with col1:
            ...
            sex_male = st.radio("الجنس", options=["1", "0"],
                                format_func=lambda x: "ذكر" if x == "1" else "أنثى",
        ...
        if st.button("تحليل مخاطر القلب"):
            with st.spinner("جاري تحليل المخاطر..."):
                ...
                if "عالية" in prediction:
                    ...
                    - متابعة الفحص الدوري سنويًا
                    - ممارسة الرياضة بانتظام
                    - تناول غذاء متوازن
                    - تجنب التدخين
                    """)

    # صفحة مراقبة المؤشرات الحيوية
